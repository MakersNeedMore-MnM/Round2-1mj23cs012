"""
Drishti-Kavach: Comprehensive Model Accuracy & Benchmark Suite

Evaluates:
  1. Universal BiSeNetV2 Track & Rail Segmentation Engine (IoU, mIoU, Dice, Pixel Acc, Latency)
  2. Custom YOLO11m Railway Sabotage & Obstacle Detector (mAP@50, mAP@50-95, Precision, Recall)
  3. Complementary Base Foundation Detector (19 COCO Classes)
  4. Domain breakdown (RailSem19 Locomotive Cab vs Indian UAV-RSOD V1, Day vs Active NIR Night)

Usage:
  # Fast benchmark (50 samples per subset):
...
class SegmentationEvaluator:
    """Computes exact pixel-level confusion matrix and segmentation metrics."""
    ...
    def compute_metrics(self) -> Dict:
        ...
        sum_row = np.sum(cm, axis=1).astype(float)
        sum_col = np.sum(cm, axis=0).astype(float)
        ...
        # 3. Class Accuracy / Recall & Mean Pixel Accuracy
...
def evaluate_segmentation_subset(
    ...
    for img_p in tqdm(image_paths, desc=desc, unit="img", leave=False):
...
def run_accuracy_benchmark(
    ...
    base_det_path: str = "models/yolo11m.pt",
    output_report_path: str = "outputs/accuracy_test_results.txt",
    samples: int = 50
):
    ...
    log("=" * 80)
    log(" 🛡️  DRISHTI-KAVACH: QUANTITATIVE ACCURACY & BENCHMARK SUITE")
    log("=" * 80)
    log(f" • Generated At:       {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    log(f" • Segmentation Model: {seg_model_path}")
    log(f" • Custom Det Model:   {det_model_path}")
    log(f" • Base Det Model:     {base_det_path}")
    log("=" * 80)
    ...
    if not os.path.exists(seg_model_path):
        log(f"[!] Segmentation weights not found at: {seg_model_path}")
    else:
        ...
        if combined_all:
            eval_count = min(samples, len(combined_all)) if samples else len(combined_all)
            log(f"\n[*] Evaluating Universal Validation Distribution (Evaluating {eval_count} Frames)...")
            m_comb, lat_comb = evaluate_segmentation_subset(model_seg, combined_all, device, "Universal Set", samples)
            ...
            log(f"   [1] Track Bed (Drivable Gauge):{m_comb['ious'][1]*100:6.2f}% IoU | Recall: {m_comb['recalls'][1]*100:5.2f}% | Precision: {m_comb['precisions'][1]*100:5.2f}%")
        ...
        benchmarks = [
            ...
            ("Locomotive Cab (Active NIR Night)", rs19_night),
            ("Indian Infrastructure (UAV-RSOD All)", uav_all),
            ("Indian Infrastructure (Daylight RGB)", uav_day),
            ("Indian Infrastructure (Active NIR Night)", uav_night),
        ]

        log("\n" + "-" * 88)
        log(f"{'Domain / Illumination Subset':<40} | {'Frames':<6} | {'mIoU (%)':<8} | {'Track Bed (%)':<13} | {'Rails (%)':<9}")
        log("-" * 88)

        for label, img_list in benchmarks:
            if img_list:
                m_sub, _ = evaluate_segmentation_subset(model_seg, img_list, device, label, max_samples=samples)
                log(f"{label:<40} | {len(img_list):<6} | {m_sub['miou']*100:6.2f}%  | {m_sub['ious'][1]*100:11.2f}%   | {m_sub['ious'][2]*100:7.2f}%")
        log("-" * 88)
    ...
    if not os.path.exists(det_model_path):
        log(f"[!] Obstacle weights '{det_model_path}' not found.")
    elif not os.path.exists(yaml_path):
        log(f"[!] Dataset config '{yaml_path}' not found.")
    else:
        from ultralytics import YOLO
        log(f"[+] Loading Custom YOLO11m Obstacle Detector from: {det_model_path}")
    ...
    print("\n" + "=" * 80)
    print(f" 📄 ACCURACY TEST RESULTS SAVED TO: {output_report_path}")
    print("=" * 80)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Drishti-Kavach Accuracy Metrics Benchmark Suite")
    ...
    parser.add_argument("--base-det", type=str, default="models/yolo11m.pt", help="Base YOLO11m weights")
    parser.add_argument("--output", type=str, default="outputs/accuracy_test_results.txt", help="Report output file")
    parser.add_argument("--samples", type=int, default=50, help="Number of validation frames per subset (default: 50)")
    parser.add_argument("--full", action="store_true", help="Evaluate on 100% of validation frames")

    args = parser.parse_args()
    sample_limit = None if args.full else args.samples

    run_accuracy_benchmark(
        seg_model_path=args.seg_model,
        det_model_path=args.det_model,
        base_det_path=args.base_det,
