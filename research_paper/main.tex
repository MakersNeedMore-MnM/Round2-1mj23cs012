...
\subsection{Atmospheric Optical Enhancer and Weather Processing}
Corridors across the Indian Railways network frequently experience severe visual degradation caused by dense winter radiation fog, heavy monsoon downpours, and low-contrast twilight illumination. Under these conditions, direct contrast loss causes thin steel rails and track obstacles to dissolve into background haze. To preserve perception reliability without incurring unnecessary compute overhead, Drishti-Kavach implements an adaptive atmospheric optical enhancement pipeline composed of four stages:

\subsubsection{Dynamic Fog Density Scoring}
Rather than applying computationally intensive restoration indiscriminately to every frame, the engine evaluates atmospheric scattering in real time. Fog density is calculated by coupling scene contrast variance with dark-channel airlight accumulation:
\begin{equation}
...
\end{equation}
where $\sigma_{\text{gray}}$ is the standard deviation of grayscale luminance representing global dynamic range, and $\mu_{\text{dark}}$ is the mean intensity of the lowest pixel values across local patches. The resulting score $S_{\text{fog}} \in [0.0, 1.0]$ gates the subsequent enhancement routines.

\subsubsection{Luminance-Domain Adaptive Contrast Enhancement (CLAHE)}
Standard RGB histogram equalization alters chromatic ratios, introducing severe color distortion. When moderate haze is detected ($0.40 < S_{\text{fog}} \le 0.60$), the system converts the input frame to the CIE $L^*a^*b^*$ color space. Contrast Limited Adaptive Histogram Equalization is applied exclusively to the luminance ($L^*$) channel:
\begin{equation}
...
\subsubsection{Physical Scattering Model Inversion (Dark Channel Prior Dehazing)}
...
\end{equation}
Atmospheric light $\mathbf{A}$ is estimated by selecting the top $0.1\%$ brightest pixels within the dark channel. The transmission map $t(\mathbf{x})$ is computed and constrained by a lower bound $t_0 = 0.10$ to prevent division by zero in dense fog pockets:
\begin{equation}
...
\subsubsection{Temporal Rain-Streak Rolling Median Filter}
...
\begin{minipage}{0.485\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/fig_bisenet_preview_1.jpg}
    \centerline{\footnotesize (a) Multi-Track Cab-View Drivable Ballast Segmentation}
\end{minipage}\hfill
\begin{minipage}{0.485\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/fig_bisenet_preview_2.jpg}
    \centerline{\footnotesize (b) High-Curvature Rail Ribbon Boundary Delineation}
\end{minipage}
\caption{BiSeNetV2 real-time semantic track bed and running rail ribbon segmentation across complex multi-track corridors and curved alignments.}
\label{fig:bisenet_seg_previews}
\end{figure*}
...
\begin{minipage}{0.485\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/fig_hud_ir.jpg}
    \centerline{\footnotesize (d) Active 850nm NIR Night Vision Telemetry HUD}
\end{minipage}
\caption{Drishti-Kavach end-to-end operational runs illustrating the 3-tier threat classification hierarchy and 24/7 all-weather performance: (a) all-clear safe track state, (b) 65px dynamic lateral caution advisory, (c) in-gauge emergency brake trigger, and (d) active 850nm NIR night-vision perception.}
...
\begin{itemize}
    ...
    \item \textbf{Active NIR Night Vision (Fig.~\ref{fig:results_gallery}(d)):} Real-time track boundary extraction and hazard clearance assessment under zero-light conditions via active 850nm NIR illumination.
\end{itemize}
